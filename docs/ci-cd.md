...
| Qualys SSL Labs | TLS/cipher/certificate grade — an independent scorecard alongside `tls-check.yml`'s testssl.sh sweep above | A+ | [ssllabs.com/ssltest](https://www.ssllabs.com/ssltest/analyze.html?d=danielmala.co) |
| Security Headers | HTTP security header grade (A+–F) — an independent scorecard alongside `headers.yml`'s exact-match check above | A+ | [securityheaders.com](https://securityheaders.com/?q=danielmala.co&followRedirects=on) |
...
| Hardenize | DNS/TLS/email configuration — same ground as `dns-check.yml` and `tls-check.yml` above, but as a continuously-monitored dashboard rather than a per-run check | — | [hardenize.com](https://www.hardenize.com/report/danielmala.co/1785606965) |
| DNSViz | DNSSEC chain validation and visualization, computed independently of `dns-check.yml`'s trust-chain check above | — | [dnsviz.net](https://dnsviz.net/d/danielmala.co/dnssec/) |
